...
# Hist plot Mito Prefilter

pdf(paste0(output_dir,"Prefilter_mito_hist_plots.pdf"))
for (sample in samples){
hist(scrna.list_prefilter[[sample]]$percent.mito, xlab="Mitochondrial proportion (%)", 
...
plots <- list()
for (sample in samples){
     plots[[sample]] <- scrna.list_prefilter[[sample]]@meta.data %>% ggplot(aes(x=nUMI, y=nGene, color=percent.mito)) + 
     geom_point() + 
...
# Hist Mito Postfilter
pdf(paste0(output_dir,"Postfilter_mito_hist_plots.pdf"))
for (sample in samples){
hist(scrna.list_postfilter[[sample]]$percent.mito, xlab="Mitochondrial proportion (%)", 
                              ylab="Number of cells", breaks=100, main=sample, col="grey80")
}
dev.off()
...
plots <- list()
for (sample in samples){
     plots[[sample]] <- scrna.list_postfilter[[sample]]@meta.data %>% ggplot(aes(x=nUMI, y=nGene, color=percent.mito)) + 
     geom_point() + 
     scale_colour_gradient(low = "gray90", high = "black") +
     stat_smooth(method=lm) +
     scale_x_log10() +
...
# Hist plot ribo
pdf(paste0(output_dir,"Prefilter_ribo_hist_plots.pdf"))
for (sample in samples){
...
plots <- list()
for (sample in samples){
     plots[[sample]] <- scrna.list_prefilter[[sample]]@meta.data %>% ggplot(aes(x=nUMI, y=nGene, color=percent.ribo)) + 
     geom_point() + 
     scale_colour_gradient(low = "gray90", high = "black") +
     stat_smooth(method=lm) +
     scale_x_log10() +
...
pdf(paste0(output_dir,"Postfilter_ribo_hist_plots.pdf"))
for (sample in samples){
...
plots <- list()
for (sample in samples){
     plots[[sample]] <- scrna.list_postfilter[[sample]]@meta.data %>% ggplot(aes(x=nUMI, y=nGene, color=percent.ribo)) + 
     geom_point() + 
     scale_colour_gradient(low = "gray90", high = "black") +
     stat_smooth(method=lm) +
     scale_x_log10() +
...
# FindAllMarkers
scrna.markers <- FindAllMarkers(scrna.combined,verbose=FALSE,test.use = "MAST")
