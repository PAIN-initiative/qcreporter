...
if (length(grep("https",in_key)) > 0) {
    ...
    for (name in sheet_names){
        ...
        MarkerGenes <- read.csv(marker_genes)
...
q95_prefilter <- list()
q99_prefilter <- list()
pdf(paste0(out_dir,"Prefilter_nUMI_hist_plots.pdf"))
for (sample in samples){
...
q5_prefilter[[sample]] <- quantile(scrna.list_prefilter[[sample]]$nUMI, 0.05)
q95_prefilter[[sample]] <- quantile(scrna.list_prefilter[[sample]]$nUMI, 0.95)
q99_prefilter[[sample]] <- quantile(scrna.list_prefilter[[sample]]$nUMI, 0.99)
}
dev.off()
write.csv(q5_prefilter,paste0(out_dir,"q5_prefilter.csv"))
write.csv(q95_prefilter,paste0(out_dir,"q95_prefilter.csv"))
write.csv(q99_prefilter,paste0(out_dir,"q99_prefilter.csv"))

q99_prefilter_median <- q99_prefilter %>% unlist() %>% median()


# filter cells nUMI < q99_prefilter_median
selected_c <- WhichCells(so_postfilter, expression = nUMI < q99_prefilter_median)
so_postfilter <- subset(so_postfilter, cells = selected_c)
