...
cell_bender_mat[[sample]] <- Read_CellBender_h5_Mat(file_name = paste0(cellbender_files,sample,"_CellBender_output_file_filtered.h5"))
...
# add timepoint name
for(i in 1:length(samples)){
  sample=samples[i]; timepoint=timepoints[i];
  scrna.list[[sample]]$timepoint <- timepoint
}

# add sex name
for(i in 1:length(samples)){
  sample=samples[i]; sex=sexes[i];
...
pdf(paste0(output_dir,"prefilter_n_cells.pdf"),onefile=TRUE,height=10,width=15)
...
saveRDS(plots,paste0(output_dir,"Prefilter_Scatter_Plots.RDS"))
...
selected_c <- WhichCells(scrna.combined_prefilter, expression = nFeature_RNA > 200)
...
selected_mito <- WhichCells(data.filt, expression = percent_mito < mito_cutoff)
selected_ribo <- WhichCells(data.filt, expression = percent_ribo > ribo_cutoff)
...
pdf(paste0(output_dir,"postfilter_n_cells.pdf"),onefile=TRUE,height=10,width=15)
ggplot(data = n_cells_df, aes(x = samples,y = as.integer(NCells),fill = samples)) + geom_bar(stat="identity") + geom_text(aes(label=NCells), position=position_dodge(width=0.9), vjust=-0.25) + ggtitle("Number of Cells") 
dev.off()
...
# FindAllMarkers
scrna.markers <- FindAllMarkers(scrna.combined,verbose=FALSE,test.use = "MAST",min.cells.feature = 0, min.cells.group = 0,
                                logfc.threshold = 0, min.pct = 0, ebayes = FALSE)
...
final_df <- data.frame(           "cluster"    = scrna.markers_sig$cluster,
                                  ...
                                  "avg_log2FC" = scrna.markers_sig$avg_log2FC,FindAllMarkers(scrna.combined,verbose=FALSE,test.use = "MAST",min.cells.feature = 0, min.cells.group = 0,
                                logfc.threshold = 0, min.pct = 0, ebayes = FALSE)
