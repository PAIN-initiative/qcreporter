...
for (sample in samples){
     ...
     scrna.list_prefilter[[sample]] <- subset(scrna.list[[sample]], cells = selected_c[[sample]])
}
...
for (sample in samples){
 sweep.res[[sample]] <- paramSweep_v3(scrna.list[[sample]]) 
...
for (sample in samples){
    ...
    scrna.list[[sample]] <- suppressMessages(doubletFinder_v3(scrna.list[[sample]], pN = 0.25, pK = as.integer(pk_value[[sample]]), nExp =     
...
saveRDS(harmonized_seurat,file.path(paste0(out_dir,"harmony"))
```
<a  id="elbow_plot"></a> 
```{r, echo=FALSE,results='hide',fig.keep='all'}
...
res_0.2 <- DimPlot(harmonized_seurat, group.by = "SCT_snn_res.0.2", label = TRUE)
pdf(paste0(out_dir, "res_0.2.pdf"))
print(res_0.2 + coord_fixed() + ggtitle("Resolution 0.2"))
mute <- dev.off()
print(res_0.2 + coord_fixed())
res_0.4 <- DimPlot(harmonized_seurat, group.by = "SCT_snn_res.0.4", label = TRUE)
pdf(paste0(out_dir, "res_0.4.pdf"))
print(res_0.4 + coord_fixed() + ggtitle("Resolution 0.4"))
mute <- dev.off()
res_0.6 <- DimPlot(harmonized_seurat, group.by = "SCT_snn_res.0.6", label = TRUE)
pdf(paste0(out_dir, "res_0.6.pdf") )
print(res_0.6 + coord_fixed() + ggtitle("Resolution 0.6"))
mute <- dev.off()
print(res_0.6 + coord_fixed())
res_0.8 <- DimPlot(harmonized_seurat, group.by = "SCT_snn_res.0.8", label = TRUE)
pdf(paste0(out_dir, "res_0.8.pdf"))
print(res_0.8 + coord_fixed() + ggtitle("Resolution 0.8"))
mute <- dev.off()
res_1 <- DimPlot(harmonized_seurat, group.by = "SCT_snn_res.1", label = TRUE)
pdf(paste0(out_dir, "res_1.pdf") )
print(res_1 + coord_fixed() + ggtitle("Resolution 1"))
mute <- dev.off()
res_1.2 <- DimPlot(harmonized_seurat, group.by = "SCT_snn_res.1.2", label = TRUE)
pdf(paste0(out_dir, "res_1.2.pdf") )
print(res_1.2 + coord_fixed() + ggtitle("Resolution 1.2"))
mute <- dev.off()
print(res_1.2 + coord_fixed())
meta <- harmonized_seurat@meta.data
Idents(harmonized_seurat) <- resolution
rm(scrna.list)
```
