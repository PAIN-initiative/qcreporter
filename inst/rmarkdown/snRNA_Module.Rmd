...
selected_c <- WhichCells(snrna.combined_prefilter, expression = nFeature_RNA > 300)
selected_f <- rownames(snrna.combined_prefilter)[Matrix::rowSums(snrna.combined_prefilter) > 3]
# Apply the filter
...
for (sample in samples){
    most_expressed_plots[[sample]] <- pdf(paste0(out_dir,sample,"_most_expressed_genes.pdf"))
...
snrna.combined_prefilter <- Merge_Seurat_List(snrna.list,
                                              ...
                                              merge.data = TRUE,
                                              project = "sample_id")
saveRDS(snrna.combined_prefilter,paste0(out_dir,'snrna.combined_prefilter.rds')) # Note this is after removing cells with > 0 % Mitochondrial Transcripts.
...
UMAP_plots <- list()
for (sample in samples){
    UMAP_plots[[sample]] <-  cowplot::plot_grid(ncol = 2, DimPlot(snrna.list[[sample]], group.by = "sample_id") + NoAxes(),
    DimPlot(snrna.list[[sample]], group.by = DF.name[[sample]]) + NoAxes())
}
...
plots <- list()
for (sample in samples){
     plots[[sample]] <- snrna.list[[sample]]@meta.data %>% ggplot(aes(x=nCount_RNA, y=nFeature_RNA, color=percent.mito)) + 
     geom_point() + 
...
# Postfilter
snrna.list <- SplitObject(postfilter , split.by = "sample_id")
plots <- list()
for (sample in samples){
     plots[[sample]] <- snrna.list[[sample]]@meta.data %>% ggplot(aes(x=nCount_RNA, y=nFeature_RNA, color=percent.mito)) + 
     geom_point() + 
...
plots <- list()
for (sample in samples){
     plots[[sample]] <- snrna.list[[sample]]@meta.data %>% ggplot(aes(x=nCount_RNA, y=nFeature_RNA, color=percent.ribo)) + 
     geom_point() + 
     scale_colour_gradient(low = "gray90", high = "black") +
     stat_smooth(method=lm) +
     scale_x_log10() +
...
p <- ggarrange(UMIs_transcripts_per_cell_prefilter,UMIs_transcripts_per_cell_postfilter,ncol=1,nrow=2,widths = 25,heights = 30)
...
p <- ggarrange(genes_per_cell_pre,genes_per_cell_post,ncol=1,nrow=2,widths = 10,heights = 8)
...
# Perform integration
snrna.anchors <- FindIntegrationAnchors(object.list = snrna.list, anchor.features = features)
snrna.combined <- IntegrateData(anchorset = snrna.anchors, dims = 1:30, new.assay.name = "CCA")
rm(snrna.anchors)
...
snrna.combined <- FindClusters(snrna.combined, graph.name = "CCA_snn", resolution = seq(0.5,3,by=0.1),algorithm = alg,verbose=FALSE)
snrna.combined <- RunUMAP(snrna.combined, reduction = "pca", dims = 1:20, n.components = 2, n.neighbors = 30,
    n.epochs = 200, min.dist = 0.3, learning.rate = 1, spread = 1)
res_0.5 <-  scCustomize::DimPlot_scCustom(snrna.combined, group.by = "CCA_snn_res.0.5", label = TRUE) + ggtitle("louvain_0.5")
res_1   <- scCustomize::DimPlot_scCustom(snrna.combined, group.by = "CCA_snn_res.1", label = TRUE) + ggtitle("louvain_1")
res_2   <- scCustomize::DimPlot_scCustom(snrna.combined, group.by = "CCA_snn_res.2", label = TRUE) + ggtitle("louvain_2")
pdf(paste0(out_dir,"resolution_snn_res_0.5.pdf"))
res_0.5
dev.off()
pdf(paste0(out_dir,"resolution_snn_res_res_1.pdf"))
res_1
dev.off()
pdf(paste0(out_dir,"resolution_snn_res_res_2.pdf"))
res_2
dev.off()

# Save Data
saveRDS(snrna.combined,paste0(out_dir,"snRNA_postfilter_int_cl.rds"))
```
```{r echo=FALSE,message=FALSE}
